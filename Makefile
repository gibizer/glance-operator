...
# openstack.org/glance-operator-bundle:$VERSION and openstack.org/glance-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/$(USER)/glance-operator
...
test: manifests generate fmt vet envtest ginkgo ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" \
	 $(GINKGO) --trace --randomize-all ${PROC_CMD} $(GINKGO_ARGS) ./test/functional/...
